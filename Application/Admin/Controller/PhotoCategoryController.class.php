 ...
 * boom
 */
class PhotoCategoryController extends CommonController {
	protected $table;
	
	public function __construct(){
		parent::__construct();
		$this->table = new Photo_categoryModel;
	}
	 	...
	 	if(IS_POST){
			...
			if(!$re){
				...
				if($data['level'] == '1'){
					...
					alert('添加成功',U('index_three_list',array('id'=>$data['p_id'])));
				}
			}
	 	}
		$this->display();
	  ...
	  public function del(){
		...
		if($id['level'] == 1){
			...
			foreach ($re1 as $key => $value) {
				$re2 = $this->table->where(array('p_id'=>$value))->save(array('is_del'=>1));
			...
			$re1 = $this->table->where(array('p_id'=>$id['id']))->getField('p_c_id',TRUE);
			foreach ($re1 as $key => $value) {
				$re2 = $this->table->where(array('p_c_id'=>$value))->save(array('is_del'=>1));
			}
		}else if($id['level'] == 3){
			$re = $this->table->where(array('p_c_id'=>$id['id']))->save(array('is_del'=>1));
		}
		if($re){
			if($id['level'] == '1'){
				...
				alert('添加成功',U('index_two_list',array('id'=>$id['id'])));
			}else if($id['level'] == '3'){
				alert('添加成功',U('index_three_list',array('id'=>$id['p_id'])));
			}
		}else{
			alert('删除失败',U('index'));
		}
...
//			dump($data);die;
	   ...
	   * 创建三级分类的方法
	   */
	  	public function index_three_list(){
	  		$data = $this->table->where(array('is_del'=>0,'p_id'=>$_GET['id'],'level'=>3))->select();
			foreach($data as $k=>$v){
				$data[$k]['addtime'] = date("Y-m-d H:i:s",$v['addtime']);
			}
			$this->assign('data',$data);
	   ...
	   */
	  	public function index_type_list(){
			...
			if(!$re && $re === FALSE){
				...
				alert('编辑成功',U('index_three_list',array('id'=>$_GET['p_id'])));
			}
	 	}
	  		$data = $this->table->where(array('is_del'=>0,'p_c_id'=>$_GET['id'],'level'=>3))->find();
			$this->assign('data',$data);
	  		$this->display();
	  	}
}
