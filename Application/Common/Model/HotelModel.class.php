...
//   array('h_name','','该公司已经注册',0,'unique',1), 		// 在新增的时候验证name字段是否唯一
//   array('h_name','require','请填写公司名称'), 			// 验证该字字段是否为空
...
//   array('city','require','地区必须填写'), //默认情况下用正则进行验证
	...
	public function audit($data){
		if($data['name'] == 'agree'){
			$re = $this->save(array('hotel_id'=>$data['id'],'apply_type'=>1));
		}else{
			$re = $this->save(array('hotel_id'=>$data['id'],'apply_type'=>2));
		};
		return $re;
	}
}
