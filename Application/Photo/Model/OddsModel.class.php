...
class OddsModel extends Model{
	...
	protected $tableName = 'photo_odds';
	...
	private function verify($post,$id){
		$price = M('Photo_product')->where(array('is_del'=>0,'p_p_id'=>$id))->getField('price');
		if($post['tnt'] > $price){
	...
	public function getData($id){
		$data = M('Photo_odds')->where(array('is_del'=>0,'p_p_id'=>$id))->find();
		//获得开始时间和结束时间
