...
class AlipayController extends Controller {
	...
	//doalipay方法
        /*该方法其实就是将接口文件包下alipayapi.php的内容复制过来
          然后进行相关处理
        */
	public function doalipay($id=NULL){
        ...
        $out_trade_no = $order['number'];//商户订单号 通过支付页面的表单进行传递，注意要唯一！
        $subject = $name;  //订单名称 //必填 通过支付页面的表单进行传递
        $total_fee = $order['total'];   //付款金额  //必填 通过支付页面的表单进行传递
...
if($_GET['trade_status'] == 'TRADE_FINISHED' || $_GET['trade_status'] == 'TRADE_SUCCESS') {
        ...
        echo '支付成功';
        redirect(U('Index/index'));//跳转到配置项中配置的支付成功页面；
    }else {
         M('shop_orders')->where(array('number'=>$number))->save(array('is_del'=>1));
        echo "trade_status=".$_GET['trade_status'];
        redirect(U('Index/index'));//跳转到配置项中配置的支付失败页面；
    }
}else {
    //验证失败
    M('shop_orders')->where(array('number'=>$number))->save(array('is_del'=>1));
