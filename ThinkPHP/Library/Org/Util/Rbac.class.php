...
class Rbac {
    ...
    static public function getAccessList($authId) {
        ...
        $sql    =   "select node.id,node.name from ".
                    ...
                    $table['node']." as node ".
                    "where user.admin_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=1 and node.status=1";
        $apps =   $db->query($sql);
        $access =  array();
        foreach($apps as $key=>$app) {
            ...
            $sql    =   "select node.id,node.name from ".
                    ...
                    $table['access']." as access ,".
                    $table['node']." as node ".
                    "where user.admin_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=2 and node.pid={$appId} and node.status=1";
            ...
            foreach($modules as $key=>$module) {
                ...
                $sql    =   "select node.id,node.name from ".
                    ...
                    $table['access']." as access ,".
                    $table['node']." as node ".
                    "where user.admin_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=3 and node.pid={$moduleId} and node.status=1";
                    $rs =   $db->query($sql);
                    foreach ($rs as $a){
	...
	static public function getModuleAccessList($authId,$module) {
        ...
        $sql    =   "select access.node_id from ".
                    ...
                    $table['user']." as user,".
                    $table['access']." as access ".
                    "where user.admin_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and  access.module='{$module}' and access.status=1";
        $rs =   $db->query($sql);
        $access	=	array();
        foreach ($rs as $node){
            $access[]	=	$node['node_id'];
